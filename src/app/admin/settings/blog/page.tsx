...
export default function AdminBlogPage() {
  ...
  const [form, setForm] = useState({
    title: "", excerpt: "", content: "", category: "General", author: "Solanki Vastra Bhandar Editorial", image: "", featured: false
  });
  ...
  const resetForm = () => {
    setForm({ title: "", excerpt: "", content: "", category: "General", author: "Solanki Vastra Bhandar Editorial", image: "", featured: false });
    setImageMedia([]);
  };
  ...
  return (
    <AdminLayout
      ...
      actions={
        <button onClick={() => { resetForm(); setEditing(null); setShowForm(true); }} className="px-4 py-2.5 bg-gradient-to-r from-[#C5A47E] to-[#C5A47E] text-white rounded-xl text-xs font-semibold flex items-center gap-2 shadow-lg shadow-[#C5A47E]/20">
    ...
    >
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between px-6 py-4 border-b border-[#111111]/[0.04]">
                <h2 className="font-display text-base font-bold text-[#111111]">{editing ? "Edit" : "New"} Post</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="flex items-center gap-2 text-sm text-[#111111] font-medium cursor-pointer">
                    <input type="checkbox" checked={form.featured} onChange={(e) => setForm({ ...form, featured: e.target.checked })} className="w-4 h-4 rounded text-[#C5A47E] focus:ring-[#C5A47E]" />
              ...
              <div className="px-6 py-4 border-t border-[#111111]/[0.04] flex gap-3">
                <button onClick={() => setShowForm(false)} className="flex-1 py-3 border border-[#111111]/[0.06] text-[#111111]/50 rounded-xl text-sm font-medium hover:bg-[#F8F6F3]">Cancel</button>
      ...
      {loading ? (
        <div className="flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-[#111111]/20" /></div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {posts.map((s) => (
            <motion.div key={s.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-[#111111]/[0.03] shadow-sm overflow-hidden group">
              <div className="relative h-36 bg-gradient-to-br from-[#F8F6F3] to-[#EDE8E0]">
                ...
                {s.featured && (
                  <div className="absolute top-2 left-2 bg-[#C5A47E] text-white text-[10px] px-2 py-0.5 rounded font-medium uppercase">
                    Featured
                  </div>
                )}
              </div>
              <div className="p-4 flex flex-col justify-between h-[120px]">
                <div>
                  <h3 className="text-sm font-bold text-[#111111] line-clamp-1">{s.title}</h3>
                  <p className="text-[11px] text-[#111111]/50 line-clamp-2 mt-1">{s.excerpt}</p>
                  <p className="text-[10px] text-[#111111]/30 mt-2">{s.category} • {new Date(s.createdAt).toLocaleDateString()}</p>
                </div>
          ...
          {posts.length === 0 && (
            <div className="col-span-full text-center py-16 bg-white rounded-2xl border border-[#111111]/[0.03]">
              <p className="text-sm text-[#111111]/40">No posts yet. Add your first post!</p>
            </div>
          )}
        </div>
      )}
